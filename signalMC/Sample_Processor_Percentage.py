...
def MakeHist(Chain, cut_hist, ofile, fname, weight, a, b, c, d):
#	ROOT.gInterpreter.Declare("float p = 0.0;")
#	ROOT.gInterpreter.Declare("p += 100.0;")
	Rdf_noCut = RDF(Chain)
	C = Rdf_noCut.Count()
	...
	#Rdf_PreSel = Rdf_noCut.Filter("(HLT_Photon110EB_TightID_TightIso > 0. || HLT_Photon175 >0.0)")
	#C = Rdf_PreSel.Count()
	#print(str(C.GetValue())+" Events After Trigger Requirement in "+fname+" Sample")
	...
	#Rdf_PreSel = Rdf_PreSel.Filter("selectedPatJetsAK8PFPuppi_softdropMass.size()>0")
	#C = Rdf_PreSel.Count()
	#print(str(C.GetValue())+" Events After selectedPatJets>0 in "+fname+" Sample")
	...
	Rdf = Rdf.Define("Gen_jM", "GenJetAK8_mass[jIndex]")
	Rdf = Rdf.Define("Gen_Pt", "GenJetAK8_pt[jIndex]")
	...
	Rdf_sigregion = Rdf.Filter("jM >= c && jM <= d")
	C = Rdf_sigregion.Count()
	print(str(C.GetValue())+"  Events in signal region in "+fname+" Sample")
	tot_sig_weight = float(C.GetValue()) * float(weight)

	Rdf_sigregion_pass = Rdf_sigregion.Filter("n2ddt <= 0")
	C = Rdf_sigregion_pass.Count()
	print(str(C.GetValue())+" Passing Events in signal region in "+fname+" Sample")

	pass_sig_pass = float(C.GetValue())

#	Rdf_sigregion_thin = Rdf.Filter("jM >= c && jM <= d && jPt >= a && jPt <= b")
#	C = Rdf_sigregion_thin.Count()
#	tot_sig_weight_thin = float(C.GetValue()) * float(weight)	

#	Rdf_sigregion_pass_thin = Rdf.Filter("jM >= c && jM <= d && n2ddt <= 0&& jPt >= a && jPt <= b")
#	C = Rdf_sigregion_pass_thin.Count()
#	pass_sig_weight_thin = float(C.GetValue()) * float(weight)	
	...
	h0 = Rdf.Histo1D(("h0",  "jIndex", 10, 0, 10), "jIndex", "total_weight")
	h0 = h0.Clone()
	h0.SetTitle("jIndex")
	h0.SetXTitle("jIndex")
	ofile.WriteObject(h0, "jIndex")
	...
	h1_2 = Rdf_sigregion.Histo1D(("h1_2",  ';N^{2}_{1}', 100, 0, 1.0), "N2", "total_weight")
	h1_2 = h1_2.Clone()
	h1_2.SetTitle("N2 in Sigregion")
	h1_2.SetXTitle("N2")
	ofile.WriteObject(h1_2, "N2_sigregion")
	
	c1_2 = TCanvas()
	c1_2.cd()
	h1_2.Draw()
	c1_2.SaveAs("/home/akobert/CMSSW_11_1_0_pre7/src/plots/RData_"+fname+"_N2_sigregion.png")
	c1_2.Close()
	print("Plot 2")
	...
	h2_gen = Rdf_Pass.Histo1D(("h2_gen", "GenJet Passing Softdrop Mass", 40, 0, 200), "Gen_jM", "total_weight")
	h2_gen = h2_gen.Clone()
	h2_gen.SetTitle("GenJet Passing Softdrop Mass")
	h2_gen.SetXTitle("Softdrop Mass")
	ofile.WriteObject(h2_gen, "gen_pass_soft")
	
	h2_1 = Rdf_sigregion_pass.Histo1D(("h2_1",  "Passing Sigregion", 40, 0, 200), "jM", "total_weight")
	h2_1 = h2_1.Clone()
	h2_1.SetTitle("Softdrop Mass "+str(c)+" GeV to "+str(d)+" GeV")
	h2_1.SetXTitle("Softdrop Mass")
	ofile.WriteObject(h2_1, "soft_sigregion_pass")
	
	pass_sig_pass_weight = h2_1.Integral()

	c2_1 = TCanvas()
	c2_1.cd()
	h2_1.Draw()
	c2_1.SaveAs("/home/akobert/CMSSW_11_1_0_pre7/src/plots/RData_"+fname+"_soft_sigregion_pass.png")
	c2_1.Close()
	print("Plot 2_1")
	
#	h2_1_thin = Rdf_sigregion_pass_thin.Histo1D(("h2_1_thin",  "Passing Thin Sigregion", 40, 0, 200), "jM", "total_weight")
#	h2_1_thin = h2_1_thin.Clone()
#	h2_1_thin.SetTitle("Softdrop Mass "+str(c)+" GeV to "+str(d)+" GeV and Jet pT "+str(a)+" to "+str(b)+" GeV")
#	h2_1_thin.SetXTitle("Softdrop Mass")
#	ofile.WriteObject(h2_1_thin, "soft_sigregion_pass_thin")
	

#	c2_1_thin = TCanvas()
#	c2_1_thin.cd()
#	h2_1_thin.Draw()
#	c2_1_thin.SaveAs("/home/akobert/CMSSW_11_1_0_pre7/src/plots/RData_"+fname+"_soft_sigregion_pass_thin.png")
#	c2_1_thin.Close()
	...
	h5_gen = Rdf_Pass.Histo1D(("h5_gen", "GenJet Passing Jet Pt", 40, 0, 2000), "Gen_Pt", "total_weight")
	h5_gen = h5_gen.Clone()
	h5_gen.SetTitle("GenJet Passing Jet Pt")
	h5_gen.SetXTitle("Pt")
	ofile.WriteObject(h5_gen, "gen_pass_jet_pt")
...
#	f8.Scale(1.0/9.0)
	...
	h26_w = Rdf_Fail.Histo2D(("pt_soft_fail_wide3", "Failing Jet pT vs. Softdrop Mass", 14, widebins3, 40, 0, 200), "jPt", "jM", "total_weight")
        h26_w = h26_w.Clone()
        h26_w.SetTitle("Failing Jet pT vs. Softdrop Mass")
        h26_w.SetXTitle("Jet pT")
        h26_w.SetYTitle("Softdrop Mass")
	ofile.WriteObject(h26_w, "jet_pt_soft_fail_wide3")

        h27_w = Rdf.Histo2D(("pt_soft_tot_wide3", "Total Jet pT vs. Softdrop Mass", 14, widebins3, 40, 0, 200), "jPt", "jM", "total_weight")
        h27_w = h27_w.Clone()
        h27_w.SetTitle("Total Jet pT vs. Softdrop Mass")
        h27_w.SetXTitle("Jet pT")
        h27_w.SetYTitle("Softdrop Mass")
	ofile.WriteObject(h27_w, "jet_pt_soft_tot_wide3")

        h28_w = Rdf_Pass.Histo2D(("pt_soft_pass_wide3", "Passing Jet pT vs. Softdrop Mass", 14, widebins3, 40, 0, 200), "jPt", "jM", "total_weight")
        h28_w = h28_w.Clone()
        h28_w.SetTitle("Passing Jet pT vs. Softdrop Mass")
        h28_w.SetXTitle("Jet pT")
        h28_w.SetYTitle("Softdrop Mass")
	ofile.WriteObject(h28_w, "jet_pt_soft_pass_wide3")
        ...
        h29_w = Rdf_Fail.Histo2D(("pt_soft_fail_wide4", "Failing Jet pT vs. Softdrop Mass", 15, widebins4, 40, 0, 200), "jPt", "jM", "total_weight")
        h29_w = h29_w.Clone()
        h29_w.SetTitle("Failing Jet pT vs. Softdrop Mass")
        h29_w.SetXTitle("Jet pT")
        h29_w.SetYTitle("Softdrop Mass")
        ofile.WriteObject(h29_w, "jet_pt_soft_fail_wide4")

        h30_w = Rdf.Histo2D(("pt_soft_tot_wide4", "Total Jet pT vs. Softdrop Mass", 15, widebins4, 40, 0, 200), "jPt", "jM", "total_weight")
        h30_w = h30_w.Clone()
        h30_w.SetTitle("Total Jet pT vs. Softdrop Mass")
        h30_w.SetXTitle("Jet pT")
        h30_w.SetYTitle("Softdrop Mass")
        ofile.WriteObject(h30_w, "jet_pt_soft_tot_wide4")

        h31_w = Rdf_Pass.Histo2D(("pt_soft_pass_wide4", "Passing Jet pT vs. Softdrop Mass", 15, widebins4, 40, 0, 200), "jPt", "jM", "total_weight")
        h31_w = h31_w.Clone()
        h31_w.SetTitle("Passing Jet pT vs. Softdrop Mass")
        h31_w.SetXTitle("Jet pT")
        h31_w.SetYTitle("Softdrop Mass")
        ofile.WriteObject(h31_w, "jet_pt_soft_pass_wide4")
	...
	print("CutBased passed: "+str(pid_pass/total_events * 100)+"%")
	...
	print("Passing Signal Region Events: "+str(pass_sig_pass))
	print("Weighted Passing Signal Region Events (Number): "+str(pass_sig_pass_weight))
	

def SamplePro(sample, fname, cut_hist, percentage=10, a=100, b=300, c=15, d=30):
	...
	MakeHist(Chain, cut_hist, ofile, fname, sample[0][1], a, b, c, d)

...
#	h9 = TH1F("h9", "N2DDT", 50, -.5, .5)
	...
	h21 = TH2F("jet_pt_soft", "Passing Jet pT vs. Softdrop Mass", 40, 0, 2000, 40, 0, 200)
	h21_1 = TH2F("jet_pt_soft_total", "Total Jet pT vs. Softdrop Mass", 40, 0, 2000, 40, 0, 200)
	h22 = TH2F("jet_pt_sof2", "Passing Jet pT vs. Softdrop Mass (Coarse)", 20, 0, 2000, 20, 0, 200)
	...
	for F in sample:
		...
		Rdf_noCut = RDF(Chain)
	#	Rdf_PreSel = Rdf_noCut.Filter("(HLT_Photon110EB_TightID_TightIso > 0. || HLT_Photon175 > 0) && nPhoton > 0. && nselectedPatJetsAK8PFPuppi > 0. && selectedPatJetsAK8PFPuppi_msoftdrop_nom.size()>0")
		...
		njcut += int(Rdf_PreSel.Count().GetValue())
	        ...
	        #Rdf_cflow = Rdf_cflow.Filter("PID(Photon_pt, Photon_eta, Photon_mvaID_WP90, nPhoton)")
	        ...
	        #Rdf_cflow = Rdf_cflow.Filter("JSOFT(selectedPatJetsAK8PFPuppi_pt_nom, selectedPatJetsAK8PFPuppi_eta, selectedPatJetsAK8PFPuppi_jetId,selectedPatJetsAK8PFPuppi_msoftdrop_nom, nselectedPatJetsAK8PFPuppi)")
	        #jsoft_pass += float(Rdf_cflow.Count().GetValue())
	        #jsoft_pass_slice += float(Rdf_cflow.Count().GetValue())
	        
		#Rdf_cflow = Rdf_cflow.Filter("(HLT_Photon110EB_TightID_TightIso > 0. || HLT_Photon175 >0.0)")
	        #trig_pass += float(Rdf_cflow.Count().GetValue())
	        #trig_pass_slice += float(Rdf_cflow.Count().GetValue())
		
		#Rdf_cflow = Rdf_cflow.Define("jIndex", "jet_index_define(nselectedPatJetsAK8PFPuppi, selectedPatJetsAK8PFPuppi_pt_nom, selectedPatJetsAK8PFPuppi_eta, selectedPatJetsAK8PFPuppi_msoftdrop_nom, selectedPatJetsAK8PFPuppi_jetId)")
        	#Rdf_cflow = Rdf_cflow.Define("pIndex", "photon_index_define(nPhoton, Photon_pt, Photon_eta, Photon_mvaID_WP90)")
	        ...
	        #Rdf_cflow = Rdf_cflow.Define("Rho", "rho(selectedPatJetsAK8PFPuppi_pt_nom[jIndex], selectedPatJetsAK8PFPuppi_msoftdrop_nom[jIndex])")
		...
		Rdf_Trig = Rdf.Filter("HLT_Photon110EB_TightID_TightIso > 0. || HLT_Photon175 > 0")
		trigcut += int(Rdf_Trig.Count().GetValue())

		#Rdf_Final = Rdf.Filter("N2 >= 0.0 && Rho > -7 && Rho < -2  && dR >= 2.2 && (HLT_Photon110EB_TightID_TightIso > 0. || HLT_Photon175 >0.0)")
...
#		h1.Add(t1)
#
        	t2 = Rdf.Histo1D(("t2", "Softdrop Mass", 40, 0, 200), "jM", "weight")
        	t2 = t2.Clone()
...
#
#
#        	t4 = Rdf.Histo1D(("t4", "Photon Pt", 40, 0, 1000), "pPt", "weight")
#	        t4 = t4.Clone()
#	        t4.SetTitle("Photon Pt")
	#        t4.SetXTitle("Pt")
#		h4.Add(t4)
	        
#		t5 = Rdf.Histo1D(("t5", "Jet Pt", 40, 0, 2000), "jPt", "weight")
#	        t5 = t5.Clone()
#	        t5.SetTitle("Jet Pt")
#	        t5.SetXTitle("Pt")
#		h5.Add(t5)
#
#
#        	t6 = Rdf.Histo1D(("t6", "Jet Eta", 20, -2.5, 2.5), "jEta", "weight")
#        	t6 = t6.Clone()
#        	t6.SetTitle("Jet Eta")
#        	t6.SetXTitle("Eta")
#		h6.Add(t6)
...
#
#
#        	t8 = Rdf.Histo1D(("t8", "Photon Eta", 20, -2.5, -2.5), "pEta", "weight")
#        	t8 = t8.Clone()
...
#
		...
		j11 =  Rdf.Histo2D(("n2_n2ddt", "N2 vs. N2DDT", 25, 0, .5, 50, -.5, .5), "N2", "n2ddt", "weight")
	        j11 = j11.Clone()
		h11.Add(j11)

	        j12 = Rdf.Histo1D(("N2",  ';N^{2}_{1}', 25, 0, .5), "N2", "weight")
        	j12 = j12.Clone()
		h12.Add(j12)
		
		j13 = Rdf.Histo1D(("N2DDT",  "N2DDT", 50, -.5, .5), "n2ddt", "weight")
	        j13 = j13.Clone()
		h13.Add(j13)
		
		j14 = Rdf.Histo2D(("n2_soft", "N2 vs. Softdrop Mass", 25, 0, .5, 40, 0, 200), "N2", "jM", "weight")
        	j14 = j14.Clone()
		h14.Add(j14)

		j15 = Rdf.Histo2D(("n2ddt_soft", "N2DDT vs. Softdrop Mass", 50, -.5, .5, 40, 0, 200), "n2ddt", "jM", "weight")
        	j15 = j15.Clone()
		h15.Add(j15)
		
		j15_1 = Rdf.Histo2D(("soft_n2ddt", "Softdrop Mass vs. N2DDT", 40, 0, 200, 50, -.5, .5), "jM", "n2ddt", "weight")
        	j15_1 = j15_1.Clone()
		h15_1.Add(j15_1)

                j16 = Rdf.Histo2D(("rho_soft", "Rho vs. Softdrop Mass", 28, -8, -1, 40, 0, 200), "Rho", "jM", "weight")
                j16 = j16.Clone()
                h16.Add(j16)

                j19 = Rdf.Histo2D(("n2_pt", "N2 vs. Jet pT", 25, 0, .5, 40, 0, 2000), "N2", "jPt", "weight")
                j19 = j19.Clone()
                h19.Add(j19)

                j20 = Rdf.Histo2D(("n2ddt_pt", "N2DDT vs. Jet pT", 50, -.5, .5, 40, 0, 2000), "n2ddt", "jPt", "weight")
                j20 = j20.Clone()
                h20.Add(j20)
		
		j20_1 = Rdf.Histo2D(("pt_n2ddt", "Jet pT vs. N2DDT", 40, 0, 2000, 50, -.5, .5), "jPt", "n2ddt", "weight")
        	j20_1 = j20_1.Clone()
		h20_1.Add(j20_1)
		
		j21 = Rdf_Pass.Histo2D(("pt_soft", "Passing Jet pT vs. N2DDT", 40, 0, 2000, 40, 0, 200), "jPt", "jM", "weight")
        	j21 = j21.Clone()
		h21.Add(j21)
		
		j21_1 = Rdf.Histo2D(("pt_soft_tot", "Total Jet pT vs. N2DDT", 40, 0, 2000, 40, 0, 200), "jPt", "jM", "weight")
        	j21_1 = j21_1.Clone()
		h21_1.Add(j21_1)
		...
		h22.Add(j22)
                ...
                t7_1 = Rdf.Histo1D(("t7_1", "Rho", 28, -8, -1), "Rho", "weight")
                t7_1 = t7_1.Clone()
                t7_1.SetTitle("Rho")
                t7_1.SetXTitle("Rho")
                h7_1.Add(t7_1)
		...
		print("Cutflow Percentages For Slice "+str(slicenum)+": ")
        	...
        	print("MVAID passed: "+str(pid_pass_slice/total_events_slice * 100)+"%")
...
#	ofile.WriteObject(h4, "test_photon_pt")
#	ofile.WriteObject(h5, "test_jet_pt")
 #       ofile.WriteObject(h6, "test_jet_eta")
  #      ofile.WriteObject(h7, "test_rho")
   #     ofile.WriteObject(h8, "test_photon_eta")
	...
	h21.SetTitle("Passing Jet pT vs. Softdrop Mass")
        ...
        ofile.WriteObject(h21, "jet_pt_soft")

        n11 = TCanvas()
        n11.cd()
        h21.Draw("COLZ")
        n11.SaveAs("/home/akobert/CMSSW_11_1_0_pre7/src/plots/RData_"+fname+"_jet_pt_soft.png")
        n11.Close()

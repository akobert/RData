...
def Cutoff(sample, rho_bin=14, pt_bin=40, n2_bin=50):
	...
	ofile = ROOT.TFile("RData_Cutoffs_Fat2.root", "RECREATE")
	ofile.cd()
	...
	h10 = TH3F("h10",  "Examined Data", rho_bin, -7, -2, pt_bin, 200, 800, n2_bin, 0, .5)
		...
		for path, subdirs, files in os.walk(F[0]):
			for name in files:
				...
				if (File.endswith(".root")):
					...
					Chain.Add(File)
		Rdf_noCut = RDF(Chain)
	#	Rdf_PreSel = Rdf_noCut.Filter("(HLT_Photon110EB_TightID_TightIso > 0. || HLT_Photon175 > 0) && nPhoton > 0. && nFatJet > 0. && FatJet_msoftdrop.size()>0")
		Rdf_PreSel = Rdf_noCut.Filter("nPhoton > 0.")
        	Rdf_PreSel = Rdf_PreSel.Filter("nFatJet > 0.")

	#	Rdf_PreSel = Rdf_PreSel.Filter("FatJet_msoftdrop.size()>0")
		...
		Rdf_final = Rdf.Filter("jPt >= 200 && jPt<=800. && N2 >= 0.0 && Rho > -7 && Rho < -2 && dR >= 2.2 && (HLT_Photon110EB_TightID_TightIso > 0. || HLT_Photon175 >0.0)")
		
		h0 = Rdf_final.Histo3D(("a"+str(label),  "Examined Data", rho_bin, -7, -2, pt_bin, 200, 800, n2_bin, 0, .5), "Rho", "jPt", "N2", "weight")
	#	h0.Draw("COLZ")
		h0 = h0.Clone()
		label += 1
	...
	cutoffs = TH2F("cuts", "N2 Cutoffs", rho_bin, -7, -2, pt_bin, 200, 800)
        cutoffs.SetStats(0)
			...
			for j in range(pt_bin):
				...
				if temp.GetBinContent(temp.GetMaximumBin()) != 0:
					...
					if percent == 10:
						total = temp.Integral()
						k = 0
						while k <= temp.GetNbinsX():
							frac = temp.Integral(0,k)/total
							if frac >= .1:
								break
							k += 1

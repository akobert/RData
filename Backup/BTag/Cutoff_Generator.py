...
#determines cutoff bin for value of rho or pt for BTagDDT plotting
...
def Proj(i, j, btag_bin, temp, t1):
	for x in range (btag_bin):
...
def DDTpass(cut_hist, btag, pt, msoft):
	if btag - cut_hist.GetBinContent(self.bin_num(Rho(msoft, pt) ,1), self.bin_num(pt,2)) < 0:
		return true
	else:
		return false

def DDT(cut_hist, btag, pt, msoft, rdf):

	ddt =  btag - cut_hist.GetBinContent(bin_num(Rho(float(msoft), float(pt)) ,1), bin_num(float(pt),2))

	return rdf.Define("ddt", ddt)



def Cutoff(sample, rho_bin=14, pt_bin=40, btag_bin=2000):

	#gROOT.SetBatch(True)

	ofile = ROOT.TFile("RData_Cutoffs_BTag.root", "RECREATE")
	ofile.cd()
	...
	h10 = TH3F("h10",  "Examined Data", rho_bin, -8, -1, pt_bin, 0, 2000, btag_bin, -1, 1)
	...
	for F in sample:
		...
		Rdf = Rdf.Define("jM", "selectedPatJetsAK8PFPuppi_softdropMass[jIndex]/(1-selectedPatJetsAK8PFPuppi_rawFactor[jIndex])")
        	...
        	Rdf = Rdf.Define("BTag", "selectedPatJetsAK8PFPuppi_pfBoostedDoubleSecondaryVertexAK8BJetTags[jIndex]")
		...
		Rdf_final = Rdf.Filter("Rho > -7 && Rho < -2  && dR >= 2.2 && (HLT_Photon110EB_TightID_TightIso > 0. || HLT_Photon175 >0.0)")
		

		h0 = Rdf_final.Histo3D(("a"+str(label),  "Examined Data", rho_bin, -8, -1, pt_bin, 0, 2000, btag_bin, -1, 1), "Rho", "jPt", "BTag", "weight")
	#	h0.Draw("COLZ")
		h0 = h0.Clone()
		label += 1
	...
	cutoffs = TH2F("cuts", "BTag Cutoffs", rho_bin, -8, -1, pt_bin, 0, 2000)
        cutoffs.SetStats(0)
	temp = TH1F("temp", "temp btag", btag_bin, -1, 1) #temporarily stores btag for each rho/pt bin

	cut = .99

	
	#Find point where (p+1)% of event are above
	for p in range(0,80):
		print("Percentile " + str(p+1))
		cutoffs.Reset()

		percent = (p+1)
		cutoffs.SetTitle("BTag Cutoffs "+str(percent)+"% DDT")

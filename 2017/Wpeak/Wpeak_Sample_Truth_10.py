...
sys.path.insert(1, '/home/akobert/CMSSW_11_1_0_pre7/src/RData/2017/Wpeak/')
from Wpeak_Processor_Truth import *
...
if __name__ == "__main__":
	...
	cutfile = "/home/akobert/CMSSW_11_1_0_pre7/src/RData/2017/Wpeak/RData_Cutoffs_nano.root"
	cf = TFile(cutfile)
	...
	ROOT.gInterpreter.ProcessLine("auto cutoff = "+str(cut_ID)+";")
        ...
        match_code =    '''
			...
			//Creates a list of boson GEN IDs within AK8 Jet Cone, sorted closest to furthest. Returns -1 if none within cone
                        RVec<int> boson_match(int nGEN, RVec<float> GEN_eta, RVec<float> GEN_phi, float eta, float phi, RVec<float> GEN_pt, RVec<float> GEN_mass, float pt, float msd, RVec<int> GEN_pdgID)
			{
                                RVec<int> match;
                                if(nGEN == 0)
                                {
                                ...
                                {
					if(abs(GEN_pdgID[i]) < 22 || abs(GEN_pdgID[i]) > 25) //If particle is not photon, Z, W, or Higgs skip
					{
						continue;
					} 
					if(deltaR(GEN_eta[i], eta, GEN_phi[i], phi) > 0.8) //If particle is outside AK8 cone skip
					{
						continue;
					} 
                                        if((pt - GEN_pt[i])/GEN_pt[i] < 0.5 && (msd - GEN_mass[i])/GEN_mass[i] < 0.3) //If particle is within pT and mass range keep
                                        {
                                                match.push_back(i);
                                        }
                                }
                                ...
                                for(int j=0; j<(match.size()-1); j++)
                                {
                                        ...
                                        {
                                                temp = -1; 
						if(deltaR(GEN_eta[match[k]], eta, GEN_phi[match[k]], phi) > deltaR(GEN_eta[match[k+1]], eta, GEN_phi[match[k+1]], phi))
                                                {
                                                        temp = match[k];
                                                        match[k] = match[k+1];
                                                        match[k+1] = temp;

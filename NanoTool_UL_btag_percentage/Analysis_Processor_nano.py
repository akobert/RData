...
def DataPro(sample, fname, cut_hist, percentage=20):
	...
	ofile = ROOT.TFile("/home/akobert/CMSSW_11_1_0_pre7/src/RData/NanoTool_UL_btag_percentage/output/RData_" + fname + ".root", "RECREATE")
	ofile.cd()
...
#		u6.Scale(1.0/9.0)
	...
	j35_w = Rdf_Pass.Histo2D(("pt_rho_pass_wide4_thin", "Passing Jet pT vs. Rho", 15, widebins4, 28, -8, -1), "jPt", "Rho", "weight")
        ...
        j38_w = Rdf_Pass.Histo2D(("pt_soft_pass_wide5", "Passing Jet pT vs. Softdrop Mass", 19, widebins5, 40, 0, 200), "jPt", "jM", "weight")
        j38_w = j38_w.Clone()
        h38_w.Add(j38_w)

        j39_w = Rdf_Final.Histo2D(("pt_soft_tot_wide5", "Total Jet pT vs. Softdrop Mass", 19, widebins5, 40, 0, 200), "jPt", "jM", "weight")
        j39_w = j39_w.Clone()
        h39_w.Add(j39_w)

        j40_w = Rdf_Fail.Histo2D(("pt_soft_pass_wide5", "Failing Jet pT vs. Softdrop Mass", 19, widebins5, 40, 0, 200), "jPt", "jM", "weight")
        j40_w = j40_w.Clone()
        h40_w.Add(j40_w)
	...
	print(str(final)+" Events After Final Cuts (including btag and MET) in "+fname+" Sample")		
	...
	print("Final passed (Btag and MET): "+str(final/total_events * 100)+"%")

...
if __name__ == "__main__":
	...
	#XS assumed to be same as non-UL
	files = [["/cms/xaastorage/NanoAOD/2018/JUNE19/VectorZPrimeGammaToQQGamma_M100_temp/", str(59.9 * 2309.0/564737.0)]]
	...
	ROOT.gInterpreter.ProcessLine("auto cutoff = "+str(cut_ID)+";")
        ...
        ddt_code =      '''
                        ...
                        '''
        region_code = "float a=100; float b=300; float c=5; float d=20;"
        ...
        cflow_code =    '''
                        ...
                        bool PID(RVec<float> ppt, RVec<float> peta, RVec<int> pID, int nPho, RVec<float> r9)
                        {
                                for(int i=0; i<nPho; i++)
                                {
                                        if(abs(peta[i]) < 2.4 && ppt[i] > 110 && pID[i] >= 3 && r9[i] >= .1)
                                        {
                                                return true;
                                        }
                                }
	...
	percentage = 20
	fname = "100GeV_CutBased"
	SamplePro(files, fname, cut_hist, percentage)	
	
	

	print("100 GeV_CutBased Finished")

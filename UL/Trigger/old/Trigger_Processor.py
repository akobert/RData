...
#	ROOT.ROOT.EnableImplicitMT()
	...
	ROOT.gInterpreter.Declare("Double_t widebins[23] = {0, 100, 120, 140, 160, 180, 200, 220, 240, 260, 280, 300, 340, 380, 420, 460, 500, 580, 660, 740, 820, 900, 1000};")	

	h4_110 = TH1F("h4_110", "Photon pT", 22, widebins)
	h4_200 = TH1F("h4_200", "Photon pT", 22, widebins)
	h4_OR = TH1F("h4_OR", "Photon pT", 22, widebins)
	h4_notrig = TH1F("h4_notrig", "Photon pT", 22, widebins)

	h3 = TH1F("h3", "Jet pT", 100, 0, 2000)

#	h4_pt = TH1F("h4_pt", "Photon pT", 22, widebins)
	...
	for F in sample:
		...
		Rdf_PreSel = Rdf_noCut.Filter("nPhoton > 0. && nselectedPatJetsAK8PFPuppi>0")
                precut = int(Rdf_PreSel.Count())
		print(str(precut)+" Events with nPhoton > 0 in Sample")
		Rdf_PreSel = Rdf_PreSel.Define("pPt", "Photon_pt[0]")
		Rdf_PreSel = Rdf_PreSel.Define("jPt", "selectedPatJetsAK8PFPuppi_pt[0]")
		Rdf_PreSel = Rdf_PreSel.Define("pID", "Photon_mvaID_WP90")
		...
		Rdf_notrig = Rdf_PreSel.Filter("abs(pEta)<1.44 && pCut >= 4 && pR9 >= .1")
		...
		t3 = Rdf_notrig.Histo1D(("t3", "Jet pT", 100, 0, 2000), "jPt", "weight")
		t3 = t3.Clone()
		h3.Add(t3)

		t4_110 = Rdf_110.Histo1D(("t4_110", "Photon pT", 22, widebins), "pPt", "weight")
		t4_110 = t4_110.Clone()
		t4_110.SetTitle("Photon pT")
		t4_110.SetXTitle("Photon pT")
		h4_110.Add(t4_110)
		
		t4_200 = Rdf_200.Histo1D(("t4_200", "Photon pT", 22, widebins), "pPt", "weight")
		t4_200 = t4_200.Clone()
		t4_200.SetTitle("Photon pT")
		t4_200.SetXTitle("Photon pT")
		h4_200.Add(t4_200)
		
		t4_notrig = Rdf_notrig.Histo1D(("t4_notrig", "Photon pT", 22, widebins), "pPt", "weight")
		t4_notrig = t4_notrig.Clone()
		t4_notrig.SetTitle("Photon pT")
		t4_notrig.SetXTitle("Photon pT")
		h4_notrig.Add(t4_notrig)
		
		t4_OR = Rdf_OR.Histo1D(("t4_OR", "Photon pT", 22, widebins), "pPt", "weight")
		t4_OR = t4_OR.Clone()
...
#		t4_pt = Rdf_pt.Histo1D(("t4_pt", "Photon pT", 22, widebins), "pPt", "weight")
#		t4_pt = t4_pt.Clone()

...
#	h4_pt = TH1F("h4_pt", "Photon pT", 25, widebins)
	...
	for F in sample:
		...
		Rdf_PreSel = Rdf_PreSel.Define("pHOE", "Photon_hoe[0]")
		Rdf_PreSel = Rdf_PreSel.Define("pSie", "Photon_sieie[0]")
		Rdf_PreSel = Rdf_PreSel.Define("pR9", "Photon_r9[0]")
		Rdf_PreSel = Rdf_PreSel.Define("weight", F[1])
		
		
		#Rdf_notrig = Rdf_PreSel.Filter("abs(pEta)<1.44 && pIso < .2 && pCut >= 4 && HLT_Mu50 > 0")
		Rdf_notrig = Rdf_PreSel.Filter("abs(pEta)<1.44 && pIso < .2 && pCut >= 4 && HLT_Mu50 > 0 && pR9 >= .1")
